...
from services.tracker_service import STATUS_ICONS, STATUSES, TrackerService
...
def _render_application(record):
    ...
    with st.container(border=True):
        ...
        with st.expander("Details, notes & CV version used"):
            if record.job_snippet:
                st.markdown(f"**Job:** {record.job_snippet}")

            notes = st.text_area("Notes", value=record.notes, key=f"notes_{record.id}", height=80)
